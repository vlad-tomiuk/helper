  ...
  useEffect(() => {
    const onPop = () => {
      ...
      if (newPath.startsWith('/shape-generator')) {
        ...
        setPage(seg && ['blob', 'wave', 'pattern'].includes(seg) ? seg : 'home');
    ...
    return (
      <div className="App">
        ...
        <nav style={{ marginTop: '1rem' }}>
          <button onClick={() => navigate('home')}>Back to Custom Shapes</button>
        </nav>
      </div>
    );
  }
      ...
      default:
        return (
          <div className="home" style={{ padding: '2rem', textAlign: 'center' }}>
            ...
            <p style={{ marginBottom: '2rem' }}>Create beautiful SVG shapes for your designs.</p>
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              <button 
                onClick={() => navigate('blob')}
                style={{ padding: '1.5rem 2rem', fontSize: '1.2rem', cursor: 'pointer', background: 'linear-gradient(135deg, #6e8efb, #a777e3)', border: 'none', color: 'white', borderRadius: '12px' }}
              >
                Blob Generator
              </button>
              {/* 
  ...
  return (
    <div className="App">
      ...
      <nav style={{ marginTop: '2rem', padding: '1rem', borderTop: '1px solid #eee' }}>
        {page !== 'home' && <button onClick={() => navigate('shape-home')}>← All Generators</button>}
      </nav>
    </div>
  );
}
