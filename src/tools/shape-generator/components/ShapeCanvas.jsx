...
export default function ShapeCanvas({
	...
	const resolveToPercent = (value, axis = 'x') => {
		...
		try {
			...
			const normalizedValue =
				value.includes('%') || value.includes('px') || value.includes('calc')
					? value
					: `${value}%`;
			if (axis === 'x') {
			...
			const resolvedPixels = parseFloat(
				axis === 'x' ? computed.left : computed.top,
			);
	...
	const handleDeletePoint = (index, e) => {
		...
		if (type !== 'polygon' || !Array.isArray(model) || model.length <= 3)
			return;
		...
		if (type === 'polygon') {
			const pointsStr =
				Array.isArray(model) && model.length > 0
					? model.map((p) => `${p2x(p.x)},${p2y(p.y)}`).join(' ')
					: '';
			return (
				<>
					{Array.isArray(model) && model.length > 0 && (
						<polygon
							points={pointsStr}
							className="fill-none stroke-indigo-500 stroke-[0.8]"
			...
			return (
				<>
					<circle
						cx={p2x(parseVal(model.cx))}
						cy={p2y(parseVal(model.cy))}
						r={p2x(parseVal(model.r))}
						className="fill-none stroke-indigo-500 stroke-[0.8]"
						clipPath="url(#canvas-clip)"
					/>
					{showPoints && (
						<>
							...
							<RenderHandle
								id="radius"
								x={parseVal(model.cx) + parseVal(model.r)}
								y={parseVal(model.cy)}
								color="#ffb86c"
			...
			return (
				<>
					<ellipse
						cx={p2x(parseVal(model.cx))}
						cy={p2y(parseVal(model.cy))}
						rx={p2x(parseVal(model.rx))}
						ry={p2y(parseVal(model.ry))}
						className="fill-none stroke-indigo-500 stroke-[0.8]"
						clipPath="url(#canvas-clip)"
					/>
					{showPoints && (
						<>
							...
							<RenderHandle
								id="rx"
								x={parseVal(model.cx) + parseVal(model.rx)}
								y={parseVal(model.cy)}
							...
							<RenderHandle
								id="ry"
								x={parseVal(model.cx)}
								y={parseVal(model.cy) + parseVal(model.ry)}
			...
			return (
				<>
					<rect
						x={p2x(parseVal(model.l))}
						y={p2y(parseVal(model.t))}
						width={p2x(100 - parseVal(model.l) - parseVal(model.r))}
						height={p2y(100 - parseVal(model.t) - parseVal(model.b))}
						className="fill-none stroke-indigo-500 stroke-[0.8]"
						clipPath="url(#canvas-clip)"
					/>
					{showPoints && (
						<>
							<RenderHandle
								id="top"
								x={(parseVal(model.l) + (100 - parseVal(model.r))) / 2}
								y={parseVal(model.t)}
								color={getPointColor(0)}
								cursor="cursor-ns-resize"
							/>
							<RenderHandle
								id="right"
								x={100 - parseVal(model.r)}
								y={(parseVal(model.t) + (100 - parseVal(model.b))) / 2}
	...
	const getClipPathStr = () => {
		...
		switch (type) {
			case 'circle':
				return `circle(${formatCssValue(model.r)} at ${formatCssValue(model.cx)} ${formatCssValue(model.cy)})`;
			case 'ellipse':
				return `ellipse(${formatCssValue(model.rx)} ${formatCssValue(model.ry)} at ${formatCssValue(model.cx)} ${formatCssValue(model.cy)})`;
			case 'inset':
				return `inset(${formatCssValue(model.t)} ${formatCssValue(model.r)} ${formatCssValue(model.b)} ${formatCssValue(model.l)})`;
			case 'polygon':
			default:
			...
			<div className="flex items-center justify-between px-4 py-2 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1e1e1e] z-10 shadow-sm">
				<div className="flex items-center gap-1">
					<button
						...
						className={cn(
							'p-1.5 rounded-md transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500',
