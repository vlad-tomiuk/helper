...
import { ArrowLeft, Shuffle, Download, Copy, Code, Maximize2, Minimize2, Activity, Zap, Circle, Square, Droplet, RefreshCw } from 'lucide-react';
...
const UniquenessHighIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    <path d="M12 2L14.5 9L22 9L16 14L18.5 21L12 17L5.5 21L8 14L2 9L9.5 9L12 2Z" strokeLinejoin="round" />
  </svg>
);
...
export default function BlobGenerator() {
  ...
  const roll = () => {
    ...
    if (useGradient) {
        const colors = ['#FF0066', '#00CC99', '#6600FF', '#FF9900', '#33CCFF', '#F7B733', '#FC4A1A'];
  ...
  const gradientObj = useGradient && !isOutline ? {
      id: 'blobGradient',
      start: gradientStart,
      end: gradientEnd
  } : null;
             ...
             <div className="flex flex-col gap-6 order-2 xl:order-1 bg-white dark:bg-[#1e1e1e] p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800">
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm text-gray-500 dark:text-gray-400 font-mono bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">{points}</span>
                    </div>
                    <div className="flex items-center gap-3">
                         <div className="text-gray-400 dark:text-gray-500"><ComplexityLowIcon /></div>
                         <input
                            type="range"
                            min="3"
                            max="12"
                        ...
                        />
                        <div className="text-gray-600 dark:text-gray-300"><ComplexityHighIcon /></div>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm text-gray-500 dark:text-gray-400 font-mono bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">{variance.toFixed(2)}</span>
                    </div>
                    <div className="flex items-center gap-3">
                         <div className="text-gray-400 dark:text-gray-500"><UniquenessLowIcon /></div>
                         <input
                            type="range"
                            min="0"
                            max="0.8"
                            ...
                            className="flex-1 h-2 bg-gray-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-indigo-600 dark:accent-indigo-400"
                        />
                        <div className="text-gray-600 dark:text-gray-300"><UniquenessHighIcon /></div>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    <label className="font-semibold text-gray-700 dark:text-gray-300">{t('shapeGenerator.style')}</label>
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        >
                             <Square size={18} className={!isOutline ? "fill-indigo-600 dark:fill-indigo-400 stroke-none" : "fill-current text-gray-500"} />
                             <span className={!isOutline ? "font-medium text-indigo-700 dark:text-indigo-300" : "text-gray-600 dark:text-gray-400"}>{t('shapeGenerator.solid')}</span>
                        </button>
                        <button 
                            onClick={() => setIsOutline(true)}
                            className={`flex items-center justify-center gap-2 py-3 rounded-xl border transition-all ${isOutline ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-600 dark:border-indigo-400' : 'bg-white dark:bg-[#252525] border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}
                        >
                            <Circle size={18} className={isOutline ? "stroke-indigo-600 dark:stroke-indigo-400" : "stroke-current text-gray-500"} />
                            <span className={isOutline ? "font-medium text-indigo-700 dark:text-indigo-300" : "text-gray-600 dark:text-gray-400"}>{t('shapeGenerator.outline')}</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    <div className="flex justify-between items-center">
                         <label className="font-semibold text-gray-700 dark:text-gray-300">{t('shapeGenerator.appearance')}</label>
                         <div className="flex items-center gap-3">
                            <button
                                onClick={resetColor}
                            ...
                            {!isOutline && (
                                ...
                                >
                                    {useGradient ? t('shapeGenerator.switchToSolid') : t('shapeGenerator.switchToGradient')}
                                </button>
                            )}
                         </div>
                    </div>
                    ...
                    {!isOutline && useGradient ? (
                        <div className="flex gap-2 p-1 bg-gray-100 dark:bg-[#252525] rounded-xl">
                            <div className="flex-1 relative h-10 overflow-hidden rounded-lg group">
                                <input type="color" value={gradientStart} onChange={e => setGradientStart(e.target.value)} className="absolute inset-0 w-full h-full cursor-pointer opacity-0" />
                                <div className="absolute inset-0 pointer-events-none flex items-center justify-center text-xs font-mono font-medium text-gray-700 dark:text-gray-200" style={{backgroundColor: gradientStart}}>{gradientStart.toUpperCase()}</div>
                            </div>
                            <div className="flex items-center justify-center text-gray-400"><ArrowLeft size={14} className="rotate-180" /></div>
                            <div className="flex-1 relative h-10 overflow-hidden rounded-lg group">
                                <input type="color" value={gradientEnd} onChange={e => setGradientEnd(e.target.value)} className="absolute inset-0 w-full h-full cursor-pointer opacity-0" />
                                <div className="absolute inset-0 pointer-events-none flex items-center justify-center text-xs font-mono font-medium text-gray-700 dark:text-gray-200" style={{backgroundColor: gradientEnd}}>{gradientEnd.toUpperCase()}</div>
                            </div>
                        </div>
                    ) : (
                        <div className="relative h-12 overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 group">
                            <input 
                                ...
                                onChange={(e) => isOutline ? setStrokeColor(e.target.value) : setFillColor(e.target.value)}
                            ...
                            <div 
                                ...
                                style={{ backgroundColor: isOutline ? strokeColor : fillColor }}
                            >
